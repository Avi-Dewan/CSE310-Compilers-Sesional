...
public:
    ...
    void printLex(FILE *logout) {
        ...
        for(int i = 0; i < num_buckets; i++) {
            ...
            if(s!= NULL) fprintf(logout, "\t%d--> ", i+1);

            while(s != NULL) {
                
               
                string firstInfo = s->getFirstInfo();

                if(firstInfo == "FUNCTION" || firstInfo == "ARRAY" ) 
                    fprintf(logout, "<%s, %s, %s> ", s->getName().c_str(), firstInfo.c_str(), s->getType().c_str());
                else
                    fprintf(logout, "<%s, %s> ", s->getName().c_str(), s->getType().c_str());
            ...
            if(scopeTable[i]!= NULL) fprintf(logout, "\n");
